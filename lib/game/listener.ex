defmodule Game.Listener do
  ...
  def acceptor do
    ...
    Logger.info fn -> "Accepting connections on port #{inspect @port}" end
    loop_acceptor(socket)
  end

  defp loop_acceptor(socket) do
    {:ok, client} = :gen_tcp.accept(socket)
    Logger.debug fn -> "Connection from #{inspect :inet.peername(client)}" end
    {:ok, pid} = Task.Supervisor.start_child(Game.TaskSupervisor, fn -> Game.Acceptor.start(client) end)
    Logger.debug fn -> "Started acceptor: #{inspect pid}" end
